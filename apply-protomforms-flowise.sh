...
if grep -q "protomforms-flowise/api/v1/" "$NGINX_CONF"; then
    ...
    read -p "   Vuoi sovrascriverla? (y/N): " response
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
        echo "❌ Operazione annullata."
        exit 0
    fi
    echo "🔄 Rimozione configurazione esistente..."
    # Rimuovi le sezioni Flowise esistenti
    sed -i '/# FLOWISE PROTOMFORMS/,/^    }$/d' "$NGINX_CONF"
    sed -i '/location \/protomforms-flowise\/assets\//,/^    }$/d' "$NGINX_CONF"
    sed -i '/location \/protomforms-flowise\/static\//,/^    }$/d' "$NGINX_CONF"
fi

# Crea il file con la configurazione Flowise
cat > /tmp/flowise_insert.txt << 'EOF'
    # ============================================
...
echo "🔍 Inserimento configurazione Flowise dopo location /api/..."

# Trova la riga dopo la chiusura di "location /api/" e inserisci prima di "location /"
# Usa awk per inserire dopo "location /api/" e prima di "location /"
awk '
    /^    location \/api\/ \{/ {
        # Stampa tutto il blocco location /api/
        print
        while (getline > 0) {
            print
            if (/^    \}$/) {
                # Dopo la chiusura, inserisci la configurazione Flowise
                print ""
                while ((getline line < "/tmp/flowise_insert.txt") > 0) {
                    print line
                }
                close("/tmp/flowise_insert.txt")
                print ""
                break
            }
        }
